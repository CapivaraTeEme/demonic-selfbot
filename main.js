// No skiddeen, ya los conosco mogolicos de mierda
...
(async () => {
   ...
   @@@            @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@             @@   
    ...
    function askToken() {
        rl.question(chalk.magentaBright('Put your token > '), async (input) => {
            token = input.trim();
    ...
    function handleNuke(client) {
        rl.question('Put Server ID > ', async (serverId) => {
            try {
                const name_c = 'destroyed-by-demonicselfbot';
                ...
                await guild.channels.create(name_c, { type: 'GUILD_TEXT' });
        ...
        rl.question('Put Server ID > ', async (serverId) => {
            rl.question('Put Message to Spam > ', (message) => {
                rl.question('Put Channel Names > ', async (channelName) => {
                    try {
                        const webhookname = 'Destroyed by DemonicS3LF';
                        const guild = await client.guilds.fetch(serverId.trim());
                        ...
                        console.log('[+] Se han creado canales y webhooks.');
